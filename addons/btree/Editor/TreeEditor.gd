...
func build_tree_from_data():
	...
	for c in data.tree.connection:
		connect_node(c.from_node, c.from_port, c.to_node, c.to_port) #ivo 4.2
	return
...
func create_node(n):
	if  n.type == 1:
		...
		var seq = sequence_scene.instantiate()
		seq.name = n.name
		seq.set_data(n.data)
		return seq
	elif n.type == 3:
		var sel = selector_scene.instantiate()
		sel.name = n.name
		sel.set_data(n.data)
		return sel
	elif n.type == 4:
		var pse = pselector_scene.instantiate()
		pse.name = n.name
		pse.set_data(n.data)
		return pse
	elif n.type == 5:
		var pc = priority_condition.instantiate()
		pc.name = n.name
		pc.set_data(n.data)
		return pc
	elif n.type == 6:
		var par = paralel_scene.instantiate()
		par.name = n.name
		par.set_data(n.data)
		return par
	elif n.type == 7:
		var mute = mute_scene.instantiate()
		mute.name = n.name
		mute.set_data(n.data)
		return mute
	elif n.type == 8:
		var rep = repeat_scene.instantiate()
		rep.name = n.name
		rep.set_data(n.data)
		return rep
	elif n.type == 9:
		var whi = while_node_scene.instantiate()
		whi.name = n.name
		whi.set_data(n.data)
		return whi
	elif n.type == 10:
		var w = wait_scene.instantiate()
		w.name = n.name
		w.set_data(n.data)
		return w
	elif n.type == 11:
		var r = race_scene.instantiate()
		r.name = n.name
		r.set_data(n.data)
		return r
	return null

func _on_TreeEditor_connection_request(from_node, from_slot, to_node, to_slot):
	for i in get_connection_list():
		if  from_node == i.from_node and from_slot == i.from_port:
			disconnect_node(i.from_node, i.from_port, i.to_node, i.to_port)
		if  to_node == i.to_node and to_slot == i.to_port:
			disconnect_node(i.from_node, i.from_port, i.to_node, i.to_port)
...
func child_delete(node):
	for i in get_connection_list():
		if  node.name == i.from_node:
			disconnect_node(i.from_node, i.from_port, i.to_node, i.to_port)
		if  node.name == i.to_node:
			disconnect_node(i.from_node, i.from_port, i.to_node, i.to_port)
	node.queue_free()
	return
...
func _on_TreeEditor_disconnection_request(from_node, from_slot, to_node, to_slot):
	disconnect_node(from_node, from_slot, to_node, to_slot)
	return

func slot_removed(name, slot):
	for i in get_connection_list():
		if  i.from_node == name and i.from_port == slot:
			disconnect_node(i.from_node, i.from_port, i.to_node, i.to_port)
	return
...
func _input(event):
	...
	if  event is InputEventKey:
		if  event.keycode == KEY_CTRL and event.pressed:
			ctrl = true
		elif event.keycode == KEY_CTRL and not event.pressed:
			ctrl = false
		
		if  event.keycode == KEY_SHIFT and event.pressed:
			shift = true
		elif event.keycode == KEY_SHIFT and not event.pressed:
			shift = false
		
		if  event.keycode == KEY_C and not event.pressed and ctrl and shift:
			copy_node()
			get_parent().hint("Recursive Duplicate Node")
		
		if  event.keycode == KEY_X and not event.pressed and ctrl and shift:
			rdelete_node()
			get_parent().hint("Recursive Delete Node")
		
		if  event.keycode == KEY_SPACE and event.pressed and ctrl and shift:
			rmove_node()
			get_parent().hint("Recursive Move Node")
		
		if  event.keycode == KEY_S and not event.pressed and ctrl:
			_on_save_pressed()
		
		if  event.keycode == KEY_F  and not event.pressed and ctrl:
			focus_selected()
...
func copy_node():
	...
	for i in nodes:
		for j in get_connection_list():
			if  i.name == j.from_node:
				connect_node(mapping[j.from_node], j.from_port, mapping[j.to_node], j.to_port)
	return
...
func rdelete_node():
	...
	for i in nodes:
		for j in get_connection_list():
			if  i.name == j.from_node:
				disconnect_node(j.from_node, j.from_port, j.to_node, j.to_port)
...
func focus_selected():
	...
	if  not selected.selected:
		return
	scroll_offset = selected.offset * zoom - ((size / 2) - (selected.size / 2))
	return
	...
	for i in get_children():
		if  i is GraphNode:
			if  most_similar == null:
				...
				most_similar = i
	if  most_similar:
		scroll_offset = most_similar.offset * zoom - ((size / 2) - (most_similar.size / 2))
	return
