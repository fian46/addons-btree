...
func label():
	var l = Label.new()
	l.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT # ivo HORIZONTAL_ALIGNMENT_RIGH
...
func _on_parallel_close_request():
	get_parent().child_delete(self)
	return


func _on_parallel_resize_request(new_minsize):
	size = new_minsize
	return
